...
const Category = () => {
  ...
  const fetchData = async () => {
    try {
      const res = await axios("http://localhost:3001/headers");
  ...
  //
  const addData = async (dataItems) => {
    try {
      const response = await axios.post("http://localhost:3001/headers", dataItems);
    ...
    <Layout>
      ...
      <div className="">
        <div className="container mx-auto w-full p-2 ">
          ...
          {form && <Form setForm={handleToggleClose} addData={addData} />}
        </div>
      </div>
    </Layout>
  );
