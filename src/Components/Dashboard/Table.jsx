      ...
      ...columns,
      {
        ...
        Cell: ({ row }) => (
          <div className="p-1 lg:text-[16px] sm:text-[12px] md:text-[14px] max-sm:text-[10px] sm:p-0">
            <button
              className="md:px-1   text-blue-400 max-sm:px-0"
              onClick={() => handleEdit(row.original)}
            >
              <FiEdit />
            </button>
            <button
              className="text-red-500 md:px-1 "
            ...
            {row.original.hasOwnProperty("employee_id") && (
              <button
                className="md:px-1   text-cyan-900"
            ...
            {row.original.hasOwnProperty("software_id") && (
              <button
                className="md:px-1   text-cyan-900"
                onClick={() => handleViewSoftware(row.original)}
              >
                <GrView />
              </button>
            )}
            {row.original.hasOwnProperty("serialNo") && (
              <button
                className="md:px-1   text-cyan-900"
                onClick={() => handleEmpListView(row.original)}
              >
                <GrView />
              </button>
            )}
...
{row.cells.map((cell, cellIndex) => (
  ...
  >
    {cell.column.id === 'status' ? (
      <div className={`rounded-full  h-fit flex-wrap px py-[4px] flex items-center justify-center ${cell.value === 'In Use' ? 'bg-red-400' : 'bg-green-300'}`}>
        <span className="text-zinc-700">{cell.value}</span>
      </div>
    ) : (
      cell.render('Cell')
    )}
